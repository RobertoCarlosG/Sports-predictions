...
async def _run() -> None:
    async with httpx.AsyncClient(timeout=60.0) as client:
        # CLI local: modo performance (carga la temporada en memoria, sin streaming).
        # El default global sigue en low_memory=True para proteger el cron de Render.
        result = await run_mlb_daily_snapshot(client, low_memory=False)
    log.info(
